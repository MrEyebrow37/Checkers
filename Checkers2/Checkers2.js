...
function addListenersForWhite() {
    ...
    piecables.forEach(function (element1) {
    element1.removeEventListener('click', handleWhiteSetup)
    })
    whitePieces.forEach(function(element2) {
        element2.addEventListener('click', handleWhiteSetup)
    }, {once: true})
}
...
function handleWhiteSetup(eW) {
    console.log('handleWhiteSetup')
    piecables.forEach(function(element) {
        element.classList.remove('potential-move')
        element.removeEventListener('click', handleWhiteMove)
        element.removeEventListener('click', handleBlackMove)
    })

    movingElementW = eW.currentTarget

    addPotentialMoveWhite(eW)

    let potentialMoves = document.querySelectorAll('.potential-move')
        potentialMoves.forEach(function(element) {
            element.addEventListener('click', handleWhiteMove, {once: true})
        })
}


function addPotentialMoveWhite(eW) {
    piecables.forEach(function(element) {
        const movingId    = parseInt(element.id.split('-')[1])
        const potentialId = parseInt(eW.currentTarget.id.split('-')[1])
        const row = parseInt(eW.currentTarget.id.split('-')[1].split(''))
...
function handleWhiteMove() {
    console.log('handleWhiteMove')
    movingElementW.classList.remove('piece-white')
    piecables.forEach(function(element) {
        element.classList.remove('potential-move')
        })
    eW.currentTarget.classList.add('piece-white')
        addListenersForWhite()
}
...
function addListenersForBlack() {
    ...
    piecables.forEach(function (element1) {
    element1.removeEventListener('click', handleBlackSetup)
    })
    blackPieces.forEach(function(element2) {
        element2.addEventListener('click', handleBlackSetup)
    }, {once: true})
}
...
function handleBlackSetup(eB) {
    ...
    movingElementB = eB.currentTarget

    addPotentialMoveBlack(eB)

    let potentialMoves = document.querySelectorAll('.potential-move')
        potentialMoves.forEach(function(element) {
            element.addEventListener('click', handleBlackMove, {once: true})
        })
}

function addPotentialMoveBlack(eB) {
    piecables.forEach(function(element) {
        const movingId    = parseInt(element.id.split('-')[1])
        const potentialId = parseInt(eB.currentTarget.id.split('-')[1])
        const row = parseInt(eB.currentTarget.id.split('-')[1].split(''))
...
function handleBlackMove(eB, movingElementB) {
    console.log('handleBlackMove')
    movingElementB.classList.remove('piece-black')
    piecables.forEach(function(element) {
        element.classList.remove('potential-move')
        })
    eB.currentTarget.classList.add('piece-black')
        addListenersForBlack()
}
